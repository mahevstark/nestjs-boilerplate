...
---
<% if (kind === 'reference' && referenceType === 'oneToMany') { -%>import { <%= name %>Entity } from '../../../../../<%= h.inflection.transform(name, ['pluralize', 'underscore', 'dasherize']) %>/infrastructure/persistence/relational/entities/<%= h.inflection.transform(name, ['underscore', 'dasherize']) %>.entity';<% } -%>